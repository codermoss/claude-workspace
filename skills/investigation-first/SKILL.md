...
### 第二步：查阅项目知识库（如果项目已初始化 Pensieve）

> "温故而知新"——先查阅已有知识，避免重复探索

在开始调查之前，先查阅项目知识库（如果存在 `<project>/.pensieve/`）：
- 检查 `knowledge/` 是否有相关的文件位置、模块边界、调用链缓存
- 检查 `decisions/` 是否有相关的架构决策记录
- 检查 `maxims/` 是否有相关的工程准则
- 检查 `pipelines/` 是否有可复用的工作流
...
2. **查阅项目知识库**（如果 `<project>/.pensieve/` 存在）：
   - 检查 `knowledge/` 中是否有相关缓存（文件位置、模块边界、调用链）
   - 检查 `decisions/` 中是否有相关架构决策
   - 检查 `maxims/` 中是否有相关工程准则
   - 如果命中：标注"已知：[来自知识库的信息]"，作为调查起点
   - 如果未命中或知识库不存在：跳过，直接进入第 3 步

3. **列调查清单**：用 checkbox 列出需要调查的具体项目，例如：
   - `- [ ]` 读取 [具体文件/模块] 了解现状
   - `- [ ]` 检查 [具体接口/配置] 了解约束条件
   - `- [ ]` 确认 [具体数据/状态] 的实际情况
